require(["osu", "underscore", "sound", "playback"], function (
   ...
   // ... Весь код initgame.js без изменений выше
   ...
   const setupTouchEvents = () => {
      if (!window.app || !window.app.view || !window.game) return;
      ...
      const touchmoveCallback = function (e) {
         if (e.touches.length > 0) {
            ...
            window.game.mouseX = ((touch.clientX - rect.left) / rect.width) * 512;
            window.game.mouseY = ((touch.clientY - rect.top) / rect.height) * 384;
      ...
      const touchstartCallback = function (e) {
         e.preventDefault();
         touchmoveCallback(e);
         window.game.M1down = true;
         window.game.down = true;
      };

      const touchendCallback = function (e) {
         e.preventDefault();
         window.game.M1down = false;
         window.game.down = false;
      };

      canvas.addEventListener("touchstart", touchstartCallback, { passive: false });
      canvas.addEventListener("touchmove", touchmoveCallback, { passive: false });
      canvas.addEventListener("touchend", touchendCallback, { passive: false });
   };

   // ⏳ Периодически проверяем, появился ли canvas
   const tryInitTouch = () => {
      if (window.app && window.app.view) {
         setupTouchEvents();
      } else {
         setTimeout(tryInitTouch, 300); // ждём появления canvas
      }
   };

   tryInitTouch(); // запускаем попытки
